...
class GameGeometry:
    ...
    def _compute_cell_gap_size(self) -> None:
        """Compute the largest cell_size and gap that fit the maze in
        the viewport."""
        self.gap = 18
    ...
    def maze_to_screen(self, pos: vec2f) -> vec2i:
        """Convert a maze grid position to the centre pixel position on
        screen."""
        x, y = pos
    ...
    def sync_maze_screen_pos(self, entity: Entity) -> None:
        """Update entity.maze_pos to reflect its current screen_pos and
        direction."""
    ...
    def get_draw_pos(self, screen_pos: vec2f) -> tuple[int, int]:
        """Return the top-left pixel coordinate at which to draw an entity
        sprite."""
        x, y = screen_pos
        return (
